...
mod tests {
    ...
    use super::*;
    use crate::client::TRAKT_API_KEY;
    use crate::client::TRAKT_API_VERSION;
    ...
    async fn all_trending_fetches_the_last_page_and_decodes_mixed_media() -> anyhow::Result<()> {
        ...
        let api = Client::with_client_id_and_base_url("test-client-id", base_url)?;
    ...
    async fn trending_returns_pagination_and_sends_requested_query() -> anyhow::Result<()> {
        ...
        let (base_url, server) = spawn_mock_server(responses).await?;
        let api = Client::with_client_id_and_base_url("query-client-id", base_url)?;
    ...
    async fn non_success_error_contains_status_and_body() -> anyhow::Result<()> {
        ...
        let (base_url, server) = spawn_mock_server(responses).await?;
        let api = Client::with_client_id_and_base_url("bad-client-id", base_url)?;
    ...
    async fn rejects_invalid_paging_before_sending_a_request() -> anyhow::Result<()> {
        let api = Client::with_client_id_and_base_url("test-client-id", "http://127.0.0.1:9")?;
