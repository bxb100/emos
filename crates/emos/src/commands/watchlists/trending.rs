use anyhow::Result;
use trakt_client::Client as TraktClient;
use trakt_client::trending::TrendingItem;
...
pub(crate) async fn generate_trending_watchlist(download_cover: bool) -> Result<()> {
    let api = TraktClient::from_env()?;
    let items = api.all_trending().await?;
    let media = rank_trending_videos(&items);
    ...
    if download_cover {
        crate::commands::covers::download_watchlist_posters(&media, COVER_NAMESPACE).await?;
    }

    write_dynamic_watchlist(OUTPUT_FILE, "Trakt 热门趋势", COVER_URL, media)
}

fn rank_trending_videos(items: &[TrendingItem]) -> Vec<WatchlistVideo> {
    ...
    ranked_items
        .into_iter()
        .enumerate()
        .map(|(rank, (title, tmdb_id, tmdb_type))| WatchlistVideo {
            tmdb_id,
            tmdb_type,
            title,
            sort: normalize_score((rank + 1) as i64, 0, last_rank),
        })
        .collect()
}
...
#[cfg(test)]
mod tests {
    use trakt_client::media::MediaIds;
    ...
    fn maps_mixed_items_and_skips_missing_tmdb_ids() {
        let items = vec![
            TrendingItem::Movie {
                watchers: 30,
                movie: trakt_client::media::Movie {
        ...
        assert_eq!(media[1].sort, 2);
    }

    #[test]
    fn handles_empty_and_single_item_rankings() {
        assert!(rank_trending_videos(&[]).is_empty());
